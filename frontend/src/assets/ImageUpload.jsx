...
const ImageUpload = () => {
  ...
  const handleImageChange = (e) => {
    ...
    setImage(file);
    setPreview(URL.createObjectURL(file));
    setResult([]); // Clear previous results on new selection
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!Array.isArray(imagePaths)) {
        alert("Error: No recommendations found or invalid response.");
        console.error("Invalid response:", res.data);
        return;
      }

      const imageUrls = imagePaths.map((path) => {
        const filename = path.split(/[/\\]/).pop(); // Handle both Windows and Unix paths
